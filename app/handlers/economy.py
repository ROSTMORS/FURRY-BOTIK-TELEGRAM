...
SHOP_SECTIONS = {
    "consumables": {"title": "🍎 Расходники", "desc": "Предметы для монет, XP и быстрых бонусов.", "items": [1, 2, 3, 5, 6, 7, 17, 18, 19]},
    "buffs": {"title": "🍀 Баффы", "desc": "Пассивные и временные усиления.", "items": [8, 10, 16]},
    "combat": {"title": "⚔️ Боевые", "desc": "Предметы для дуэлей и боевых бонусов.", "items": [14, 15, 20, 21, 22]},
    "rare": {"title": "💎 Редкие", "desc": "Дорогие, особые и статусные предметы.", "items": [9, 11, 12]},
}
...
def shop_main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🍎 Расходники", callback_data="shop_cat_consumables"),
            InlineKeyboardButton(text="🍀 Баффы", callback_data="shop_cat_buffs"),
        ],
        [
            InlineKeyboardButton(text="⚔️ Боевые", callback_data="shop_cat_combat"),
            InlineKeyboardButton(text="💎 Редкие", callback_data="shop_cat_rare"),
        ],
    ])


def shop_category_kb(section_key: str) -> InlineKeyboardMarkup:
    section = SHOP_SECTIONS.get(section_key)
    if not section:
        return shop_main_kb()
    ...
    rows.append([InlineKeyboardButton(text="⬅️ В магазин", callback_data="shop_main")])
...
def shop_main_text() -> str:
    return (
        ...
        "🍎 <b>Расходники</b> — быстрые бонусы, XP и монеты\n"
        "🍀 <b>Баффы</b> — усиления для игр и пассивки\n"
        "⚔️ <b>Боевые</b> — бонусы для дуэлей\n"
        "💎 <b>Редкие</b> — особые и статусные предметы\n\n"
        "Покупка теперь работает кнопками.\n"
        "Старый способ тоже доступен: <code>/buy номер</code>"
    )


def shop_category_text(section_key: str) -> str:
    section = SHOP_SECTIONS.get(section_key)
    if not section:
        return shop_main_text()

    clean_title = section["title"].split(" ", 1)[1] if " " in section["title"] else section["title"]
    lines = [
        f"{section['title']} <b>{clean_title}</b>",
        f"<i>{section['desc']}</i>",
        "",
    ]
    ...
    for item_id in section["items"]:
        ...
        usable = "можно использовать" if item.get("usable", True) else "пассивный / особый"
        lines.extend([
            f"<b>{item_id}. {item['name']}</b>",
            f"💰 Цена: <b>{item['price']} 🪙</b>",
            f"📌 {item['desc']}",
            f"{rarity_badge(item)} • {category_label(item)} • {usable}{passive}",
            "",
        ])
...
def _shop_section_for_item(item_id: int) -> str:
    for section_key, section in SHOP_SECTIONS.items():
        if item_id in section["items"]:
            return section_key
    return "consumables"


def _buy_item_for_user(user_id: int, item_id: int) -> tuple[bool, str]:
    ...
    if u["balance"] < item["price"]:
        return False, f"❌ Нужно {item['price']} 🪙, у вас {u['balance']} 🪙."
    ...
    update_quest_progress(user_id, "buy_item", 1)
    new_balance = get_user(user_id)["balance"]
    return True, f"✅ Куплено: {item['name']}!{bonus}\n💰 Баланс: <b>{new_balance} 🪙</b>"
...
async def cb_shop_category(call: CallbackQuery):
    section_key = call.data.replace("shop_cat_", "", 1)
    if section_key not in SHOP_SECTIONS:
        await call.answer("Категория не найдена.", show_alert=True)
        return
    await call.message.edit_text(
        shop_category_text(section_key),
        parse_mode="HTML",
        reply_markup=shop_category_kb(section_key),
    )
    await call.answer()
...
async def cb_shop_buy(call: CallbackQuery):
    try:
        item_id = int(call.data.replace("shop_buy_", "", 1))
    except ValueError:
        await call.answer("Ошибка предмета.", show_alert=True)
        return

    ensure_user(call.from_user.id, call.from_user.username or call.from_user.full_name)
    ok, result_text = _buy_item_for_user(call.from_user.id, item_id)
    if not ok:
        await call.answer(result_text, show_alert=True)
        return

    section_key = _shop_section_for_item(item_id)
    await call.answer("Покупка успешна!", show_alert=False)
    await call.message.edit_text(
        f"{shop_category_text(section_key)}\n\n{result_text}",
        parse_mode="HTML",
        reply_markup=shop_category_kb(section_key),
    )
    await check_achievements(call.from_user.id, call.message)
...
async def cmd_buy(message: Message):
    ...
    item_id = int(args[1])
    ok, result_text = _buy_item_for_user(message.from_user.id, item_id)
