...
SLOT_SYMBOLS = {
    "🍒": {"weight": 24, "pair": 1.60, "triple": 3.10, "label": "Вишни"},
    "🍋": {"weight": 24, "pair": 1.60, "triple": 3.10, "label": "Лимоны"},
    "🍊": {"weight": 24, "pair": 1.60, "triple": 3.10, "label": "Апельсины"},
    "🍇": {"weight": 24, "pair": 1.60, "triple": 3.10, "label": "Виноград"},
    "⭐": {"weight": 10, "pair": 2.00, "triple": 4.80, "label": "Звёзды"},
    "🍀": {"weight": 7,  "pair": 2.50, "triple": 6.20, "label": "Клевер"},
    "💎": {"weight": 3,  "pair": 3.00, "triple": 8.00, "label": "Алмазы"},
}
...
async def _run_slots_spin(session: dict) -> tuple[list[str], str]:
    ...
    if result["win"]:
        ...
        lines.append(f"💰 Выплата: <b>{payout} 🪙</b>")
        if free_bet:
            lines.append(f"📈 Итог: <b>+{payout} 🪙</b>")
        else:
            sign = "+" if net >= 0 else ""
            lines.append(f"📈 Итог по ставке: <b>{sign}{net} 🪙</b>")
        if xp > 0:
