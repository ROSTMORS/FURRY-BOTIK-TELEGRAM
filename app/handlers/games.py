...
async def cb_duel_accept(call: CallbackQuery):
    ...
    c_bonus = int(get_effect_value(challenger_id, "duel_roll_bonus"))
    t_bonus = int(get_effect_value(target_id, "duel_roll_bonus"))
    c_total = c_roll + c_bonus
    t_total = t_roll + t_bonus
    if c_bonus:
    ...
    if c_total == t_total:
        ...
        await call.message.edit_text(
            f"⚔️ Дуэль завершена\\!\n"
            f"🎲 {mention(c_name, challenger_id)}: {c_roll}" + (f" (+{c_bonus})" if c_bonus else "") + f" = {c_total}\n"
            f"🎲 {mention(t_name, target_id)}: {t_roll}" + (f" (+{t_bonus})" if t_bonus else "") + f" = {t_total}\n\n"
