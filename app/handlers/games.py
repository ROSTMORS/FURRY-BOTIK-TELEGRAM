...
SLOT_SYMBOLS = {
    "🍒": {"weight": 24, "pair": 1.50, "triple": 3.00, "label": "Вишни"},
    "🍋": {"weight": 24, "pair": 1.50, "triple": 3.00, "label": "Лимоны"},
    "🍊": {"weight": 24, "pair": 1.50, "triple": 3.00, "label": "Апельсины"},
    "🍇": {"weight": 24, "pair": 1.50, "triple": 3.00, "label": "Виноград"},
    "⭐": {"weight": 10, "pair": 2.20, "triple": 5.00, "label": "Звёзды"},
    "🍀": {"weight": 7,  "pair": 2.60, "triple": 6.50, "label": "Клевер"},
    "💎": {"weight": 3,  "pair": 3.40, "triple": 10.00, "label": "Алмазы"},
}
...
async def cmd_slots(message: Message):
    ...
    if not result["win"] and total_luck > 0 and random.random() < min(total_luck, 0.45):
    ...
    if result["win"]:
        ...
        extra_lines = [f"{result['title']}", f"{result['detail']}"]
        extra_lines.append(f"💰 Выплата: <b>{payout} 🪙</b>")
        if free_bet:
            extra_lines.append(f"📈 Чистый плюс: <b>+{payout} 🪙</b>")
        else:
            sign = "+" if net >= 0 else ""
            extra_lines.append(f"📈 Итог по ставке: <b>{sign}{net} 🪙</b>")
        if xp > 0:
            extra_lines.append(f"⭐ Опыт: <b>+{xp} XP</b>")
        if luck_used:
            extra_lines.append("🍀 Удача дотянула один из барабанов до выигрыша")
    ...
    footer = f"\n🍀 Активная удача: <b>{', '.join(luck_parts)}</b>" if luck_parts else ""
